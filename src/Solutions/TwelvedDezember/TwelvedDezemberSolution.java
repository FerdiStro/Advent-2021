package Solutions.TwelvedDezember;
...
public class TwelvedDezemberSolution {
     int partOne;
     int partTwo;
    ...
    private  record Node(String name, Set<String> neighbors) {
        public boolean isSmallCave() {
            return TwelvedDezemberSolution.isSmallCaveName(name());
        }
    }
    ...
    private void part1(Map<String, Node> graph) {
    ...
    private  void part2(Map<String, Node> graph) {
        Deque<Deque<Node>> search_space = new ArrayDeque<Deque<Node>>();
        search_space.addLast(new ArrayDeque<Node>());
        search_space.getLast().addLast(graph.get("start"));
        int nPaths = 0;
    ...
    public void Solution() {
        try {
            var input = Files.lines(Path.of("rsc/input12Dezember.txt"))
